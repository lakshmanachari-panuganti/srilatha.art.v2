...
import ProductActions, { ImageGallery, ProductTabs } from '@/app/product/[slug]/ProductActions';
...
function StarRating({ rating, size = '1rem' }: { rating: number; size?: string }) {
  ...
  return (
    <span style={{ fontSize: size, color: 'var(--accent-gold)' }} aria-label={`${rating} out of 5`}>
      {'★'.repeat(filled)}
      <span style={{ color: 'var(--text-dim)' }}>{'★'.repeat(5 - filled)}</span>
    </span>
  );
}
      ...
      <div className="container" style={{ paddingTop: 'var(--sp-8)' }}>
        ...
        <div className="product-detail-grid">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--sp-5)' }}>
            <span className="badge badge-gold" style={{ alignSelf: 'flex-start' }}>{categoryLabel}</span>
            ...
            {/* Rating — only shown when there is at least one approved review */}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--sp-3)', flexWrap: 'wrap' }}>
              ...
              {discountPct && <span className="badge badge-green">{discountPct}% off</span>}
            </div>
